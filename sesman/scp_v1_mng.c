...
/******************************************************************************/
void scp_v1_mng_process(struct SCP_CONNECTION *c, struct SCP_SESSION *s)
{
	long data;
	...
	if (!data)
	{
		...
		log_message(LOG_LEVEL_INFO, "[MNG] Login failed for user %s. Connection terminated", s->username);
	...
	if (0 == access_login_mng_allowed(s->username))
	{
		...
		log_message(LOG_LEVEL_INFO, "[MNG] User %s not allowed on TS. Connection terminated", s->username);
		scp_session_destroy(s);
		auth_end(data);
		return;
	}
