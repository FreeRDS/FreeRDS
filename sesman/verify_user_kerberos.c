...
typedef enum
{
	INIT_PW, INIT_KT, RENEW, VALIDATE
} action_type;
...
k5_begin(struct k_opts *opts, struct k5_data *k5, struct user_info *u_info)
{
	...
	} else
	{
		...
		if (opts->action == INIT_KT)
		{
			...
			code = krb5_sname_to_principal(k5->ctx, NULL, NULL, KRB5_NT_SRV_HST, &k5->me);
		...
		krb5_prompt prompts[])
{
	...
	{
		if (types[i] == KRB5_PROMPT_TYPE_PASSWORD || types[i] == KRB5_PROMPT_TYPE_NEW_PASSWORD_AGAIN)
		{
...
static int k5_kinit(struct k_opts *opts, struct k5_data *k5, struct user_info *u_info)
{
	...
	{
		case INIT_PW:
			code = krb5_get_init_creds_password(k5->ctx, &my_creds, k5->me, 0, kinit_prompter, u_info,
					opts->starttime, opts->service_name, &options);
			break;
		case INIT_KT:
			code = krb5_get_init_creds_keytab(k5->ctx, &my_creds, k5->me, keytab, opts->starttime,
		...
		case VALIDATE:
			...
			break;
		case RENEW:
			code = krb5_get_renewed_creds(k5->ctx, &my_creds, k5->me, k5->cc, opts->service_name);
			break;
	}
