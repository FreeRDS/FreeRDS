...
xrdp_mcs_create(struct xrdp_sec *owner, struct trans *trans, struct stream *client_mcs_data,
...
xrdp_mcs_send_cjcf(struct xrdp_mcs *self, int userid, int chanid)
{
	...
	{
		free_stream(s); DEBUG(("  out xrdp_mcs_send_cjcf error"));
		return 1;
	}
	...
	free_stream(s); DEBUG(("  out xrdp_mcs_send_cjcf"));
	return 0;
}
...
/* returns error */
static int APP_CC
xrdp_mcs_ber_parse_header(struct xrdp_mcs *self, struct stream *s, int tag_val, int *len)
{
	int tag;
	int l;
	int i;
...
xrdp_mcs_send_aucf(struct xrdp_mcs *self)
{
	...
	if (xrdp_iso_init(self->iso_layer, s) != 0)
	{
		free_stream(s); DEBUG(("  out xrdp_mcs_send_aucf error"));
		return 1;
	}
	...
	free_stream(s); DEBUG(("  out xrdp_mcs_send_aucf"));
	return 0;
}
...
/* returns error */
static int APP_CC
xrdp_mcs_ber_out_header(struct xrdp_mcs *self, struct stream *s, int tag_val, int len)
{
	if (tag_val > 0xff)
	{
...
/* returns error */
static int APP_CC
xrdp_mcs_out_domain_params(struct xrdp_mcs *self, struct stream *s, int max_channels, int max_users, int max_tokens,
		int max_pdu_size)
{
...
xrdp_mcs_send_connect_response(struct xrdp_mcs *self)
{
	...
	if (xrdp_iso_send(self->iso_layer, s) != 0)
	{
		free_stream(s); DEBUG(("  out xrdp_mcs_send_connect_response error"));
		return 1;
	}

	free_stream(s); DEBUG(("  out xrdp_mcs_send_connect_response"));
	return 0;
}
...
xrdp_mcs_incoming(struct xrdp_mcs *self)
{
	...
	if (xrdp_mcs_send_cjcf(self, self->userid, self->userid + MCS_USERCHANNEL_BASE) != 0)
	{
		return 1;
	}
