...
int APP_CC
xrdp_orders_send_window_cached_icon(struct xrdp_orders *self, int window_id, int cache_entry, int cache_id, int flags)
{
	int order_size;
	int order_flags;
	int field_present_flags;
	...
	out_uint16_le(self->out_s, order_size);
	/* FieldsPresentFlags (4 bytes) */
	field_present_flags = flags | WINDOW_ORDER_TYPE_WINDOW | WINDOW_ORDER_CACHED_ICON;
	out_uint32_le(self->out_s, field_present_flags);
	/* windowId (4 bytes) */
	out_uint32_le(self->out_s, window_id);
	/* CacheEntry (2 bytes) */
...
/* returns error */
static int APP_CC
xrdp_orders_send_ts_icon(struct stream *s, int cache_entry, int cache_id, struct rail_icon_info *icon_info)
{
	int use_cmap;
...
/* flags can contain WINDOW_ORDER_STATE_NEW and/or
 WINDOW_ORDER_FIELD_ICON_BIG */
int APP_CC
xrdp_orders_send_window_icon(struct xrdp_orders *self, int window_id, int cache_entry, int cache_id,
		struct rail_icon_info *icon_info, int flags)
{
	...
	out_uint16_le(self->out_s, order_size);
	/* FieldsPresentFlags (4 bytes) */
	field_present_flags = flags | WINDOW_ORDER_TYPE_WINDOW | WINDOW_ORDER_ICON;
	out_uint32_le(self->out_s, field_present_flags);
	/* windowId (4 bytes) */
	out_uint32_le(self->out_s, window_id);
...
/* returns error */
int APP_CC
xrdp_orders_send_notify_delete(struct xrdp_orders *self, int window_id, int notify_id)
{
	int order_size;
	int order_flags;
	int field_present_flags;
...
/* flags can contain WINDOW_ORDER_STATE_NEW */
int APP_CC
xrdp_orders_send_notify_new_update(struct xrdp_orders *self, int window_id, int notify_id,
		struct rail_notify_state_order *notify_state, int flags)
{
	...
	if (field_present_flags & WINDOW_ORDER_ICON)
	{
		...
		order_size += 12 + notify_state->icon_info.mask_bytes + notify_state->icon_info.data_bytes;
	...
	{
		/* Icon (variable) */
		xrdp_orders_send_ts_icon(self->out_s, notify_state->icon_cache_entry, notify_state->icon_cache_id,
...
int APP_CC
xrdp_orders_send_monitored_desktop(struct xrdp_orders *self, struct rail_monitored_desktop_order *mdo, int flags)
{
	int order_size;
	int order_flags;
	int field_present_flags;
