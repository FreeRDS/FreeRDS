...
xrdp_orders_send(struct xrdp_orders *self)
{
	...
	if (self->order_level > 0)
	{
		...
		if ((self->order_level == 0) && (self->order_count > 0))
		{
			...
			if (xrdp_rdp_send_data(self->rdp_layer, self->out_s, RDP_DATA_PDU_UPDATE) != 0)
			{
				rv = 1;
			}
		}
...
xrdp_orders_last_bounds(struct xrdp_orders *self, struct xrdp_rect *rect)
{
	...
	if ((rect->left == self->orders_state.clip_left) && (rect->top == self->orders_state.clip_top) && (rect->right
			== self->orders_state.clip_right) && (rect->bottom == self->orders_state.clip_bottom))
	{
		return 1;
	}
...
xrdp_orders_out_bounds(struct xrdp_orders *self, struct xrdp_rect *rect)
{
	...
	if (rect->left == self->orders_state.clip_left)
	{
		...
		if (g_abs(rect->left - self->orders_state.clip_left) < 128)
		{
			bounds_flags |= 0x10;
		} else
		{
	...
	if (rect->top == self->orders_state.clip_top)
	{
		...
		if (g_abs(rect->top - self->orders_state.clip_top) < 128)
		{
			bounds_flags |= 0x20;
		} else
		{
	...
	if (rect->right == self->orders_state.clip_right)
	{
		...
		if (g_abs(rect->right - self->orders_state.clip_right) < 128)
		{
			bounds_flags |= 0x40;
		} else
		{
	...
	if (rect->bottom == self->orders_state.clip_bottom)
	{
		...
		if (g_abs(rect->bottom - self->orders_state.clip_bottom) < 128)
		{
			bounds_flags |= 0x80;
		} else
		{
...
/* returns error */
static int APP_CC
xrdp_order_pack_small_or_tiny(struct xrdp_orders *self, char *order_flags_ptr, int orders_flags, char *present_ptr,
		int present, int present_size)
{
	...
	if (move_up_count > 0)
	{
		...
		{
			present_ptr[index + (present_size - move_up_count)] = present_ptr[index + present_size];
		}
...
xrdp_orders_rect(struct xrdp_orders *self, int x, int y, int cx, int cy, int color, struct xrdp_rect *rect)
{
	...
	if (rect != 0)
	{
		...
		if (x < rect->left || y < rect->top || x + cx > rect->right || y + cy > rect->bottom)
		{
	...
	if ((order_flags & RDP_ORDER_BOUNDS) && !(order_flags & RDP_ORDER_LASTBOUNDS))
	{
	...
	{
		present |= 0x10;
		self->orders_state.rect_color = (self->orders_state.rect_color & 0xffff00) | (color & 0xff);
	...
	{
		present |= 0x20;
		self->orders_state.rect_color = (self->orders_state.rect_color & 0xff00ff) | (color & 0xff00);
	...
	{
		present |= 0x40;
		self->orders_state.rect_color = (self->orders_state.rect_color & 0x00ffff) | (color & 0xff0000);
		out_uint8(self->out_s, color >> 16);
	}

	xrdp_order_pack_small_or_tiny(self, order_flags_ptr, order_flags, present_ptr, present, 1);
	return 0;
}
		...
		struct xrdp_brush *brush, struct xrdp_rect *rect)
{
	...
	if (g_memcmp(brush->pattern + 1, self->orders_state.pat_blt_brush.pattern + 1, 7) != 0)
	{
		present |= 0x0800;
		out_uint8a(self->out_s, brush->pattern + 1, 7);
		g_memcpy(self->orders_state.pat_blt_brush.pattern + 1, brush->pattern + 1, 7);
	}

	xrdp_order_pack_small_or_tiny(self, order_flags_ptr, order_flags, present_ptr, present, 2);
	return 0;
}
		...
		int bg_color, struct xrdp_pen *pen, struct xrdp_rect *rect)
{
	...
	{
		/* if clip is present, still check if its needed */
		if (MIN(endx, startx) < rect->left || MIN(endy, starty) < rect->top || MAX(endx, startx)
				>= rect->right || MAX(endy, starty) >= rect->bottom)
		{
			order_flags |= RDP_ORDER_BOUNDS;
		...
		int srcx, int srcy, int cache_idx, struct xrdp_rect *rect)
{
	...
	if (cache_id != self->orders_state.mem_blt_cache_id || color_table != self->orders_state.mem_blt_color_table)
	{
		present |= 0x0001;
		...
		int box_bottom, int x, int y, char *data, int data_len, struct xrdp_rect *rect)
{
	...
	{
		/* if clip is present, still check if its needed */
		if ((box_right - box_left > 1 && (box_left < rect->left || box_top < rect->top || box_right
				> rect->right || box_bottom > rect->bottom)) || (clip_left < rect->left || clip_top
				< rect->top || clip_right > rect->right || clip_bottom > rect->bottom))
		{
			order_flags |= RDP_ORDER_BOUNDS;
	...
	xrdp_order_pack_small_or_tiny(self, order_flags_ptr, order_flags, present_ptr, present, 3);
	return 0;
}
...
int APP_CC
xrdp_orders_send_raw_bitmap(struct xrdp_orders *self, int width, int height, int bpp, char *data, int cache_id,
		int cache_idx)
{
	int order_flags = 0;
	int len = 0;
...
/* max size width * height * Bpp + 16 */
int APP_CC
xrdp_orders_send_bitmap(struct xrdp_orders *self, int width, int height, int bpp, char *data, int cache_id,
		int cache_idx)
{
	int order_flags = 0;
	int len = 0;
	...
	p = s->p;
	i = height;
	lines_sending = xrdp_bitmap_compress(data, width, height, s, bpp, 16384, i - 1, temp_s, e);
...
int APP_CC
xrdp_orders_send_raw_bitmap2(struct xrdp_orders *self, int width, int height, int bpp, char *data, int cache_id,
		int cache_idx)
{
	int order_flags = 0;
	int len = 0;
...
/* max size width * height * Bpp + 14 */
int APP_CC
xrdp_orders_send_bitmap2(struct xrdp_orders *self, int width, int height, int bpp, char *data, int cache_id,
...
/*****************************************************************************/
static int xrdp_orders_send_as_jpeg(struct xrdp_orders *self, int width, int height, int bpp, int hints)
{
	if (hints & 1)
	{
		return 0;
...
/*****************************************************************************/
static int APP_CC
xrdp_orders_out_v3(struct xrdp_orders *self, int cache_id, int cache_idx, char *buf, int bufsize, int width,
		int height, int bpp, int codec_id)
{
	int Bpp;
	int order_flags;
	int len;
...
/*  secondary drawing order (bitmap v3) using remotefx compression */
int APP_CC
xrdp_orders_send_bitmap3(struct xrdp_orders *self, int width, int height, int bpp, char *data, int cache_id,
		int cache_idx, int hints)
{
	int e;
	int bufsize;
	...
	if (ci->v3_codec_id == ci->rfx_codec_id)
	{
		...
		if (ci->v3_codec_id == ci->jpeg_codec_id)
		{
...
int APP_CC
xrdp_orders_send_brush(struct xrdp_orders *self, int width, int height, int bpp, int type, int size, char *data,
		int cache_id)
{
	int order_flags = 0;
	int len = 0;
