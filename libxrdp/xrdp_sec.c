...
xrdp_sec_create(struct xrdp_rdp *owner, struct trans *trans, int crypt_level, int channel_code)
{
	...
	self->mcs_layer = xrdp_mcs_create(self, trans, &self->client_mcs_data, &self->server_mcs_data);
...
xrdp_sec_decrypt(struct xrdp_sec *self, char *data, int len)
{
	...
	{
		xrdp_sec_update(self->decrypt_key, self->decrypt_update_key, self->rc4_key_len);
...
xrdp_sec_encrypt(struct xrdp_sec *self, char *data, int len)
{
	...
	{
		xrdp_sec_update(self->encrypt_key, self->encrypt_update_key, self->rc4_key_len);
...
xrdp_sec_process_logon_info(struct xrdp_sec *self, struct stream *s)
{
	...
	in_uint8s(s, 4);
	in_uint32_le(s, flags); DEBUG(("in xrdp_sec_process_logon_info flags $%x", flags));
...
xrdp_sec_establish_keys(struct xrdp_sec *self)
{
	...
	xrdp_sec_hash_48(temp_hash, input, self->client_random, self->server_random, 65);
	xrdp_sec_hash_48(session_key, temp_hash, self->client_random, self->server_random, 88);
	g_memcpy(self->sign_key, session_key, 16);
	xrdp_sec_hash_16(self->encrypt_key, session_key + 16, self->client_random, self->server_random);
	xrdp_sec_hash_16(self->decrypt_key, session_key + 32, self->client_random, self->server_random);
...
xrdp_sec_recv(struct xrdp_sec *self, struct stream *s, int *chan)
{
	...
	in_uint32_le(s, flags); DEBUG((" in xrdp_sec_recv flags $%x", flags));
	...
	if (flags & SEC_CLIENT_RANDOM) /* 0x01 */
	{
		...
		xrdp_sec_rsa_op(self->client_random, self->client_crypt_random, self->pub_mod, self->pri_exp);
...
xrdp_sec_process_mcs_data(struct xrdp_sec *self)
{
	...
	while (s_check_rem(s, 4))
	{
		...
		{
			g_writeln("error in xrdp_sec_process_mcs_data tag %d size %d", tag, size);
			break;
		}
		...
		switch (tag)
		{
			...
			case SEC_TAG_CLI_4:
				break;
			default:
				g_writeln("error unknown xrdp_sec_process_mcs_data tag %d size %d", tag, size);
				break;
		}
...
xrdp_sec_incoming(struct xrdp_sec *self)
{
	...
	for (index = 0; index < items->count; index++)
	{
		...
		if (g_strcasecmp(item, "pub_exp") == 0)
		{
			...
			if (g_strcasecmp(item, "pub_mod") == 0)
			{
				hex_str_to_bin(value, self->pub_mod, 64);
			} else
				if (g_strcasecmp(item, "pub_sig") == 0)
				{
					hex_str_to_bin(value, self->pub_sig, 64);
				} else
					if (g_strcasecmp(item, "pri_exp") == 0)
					{
