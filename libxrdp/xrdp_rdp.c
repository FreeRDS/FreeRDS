...
xrdp_rdp_read_config(xrdpClientInfo *client_info)
{
	...
	for (index = 0; index < items->count; index++)
	{
		...
		if (g_strcasecmp(item, "bitmap_cache") == 0)
		{
			...
			if (g_strcasecmp(item, "bitmap_compression") == 0)
			{
				client_info->use_bitmap_comp = text2bool(value);
			} else
				if (g_strcasecmp(item, "bulk_compression") == 0)
				{
					client_info->use_bulk_comp = text2bool(value);
				} else
					if (g_strcasecmp(item, "crypt_level") == 0)
					{
						if (g_strcasecmp(value, "low") == 0)
						{
							client_info->crypt_level = 1;
						} else
							if (g_strcasecmp(value, "medium") == 0)
							{
								client_info->crypt_level = 2;
							} else
								if (g_strcasecmp(value, "high") == 0)
								{
						...
						if (g_strcasecmp(item, "allow_channels") == 0)
						{
							...
							if (g_strcasecmp(item, "max_bpp") == 0)
							{
...
xrdp_rdp_create(xrdpSession *session, struct trans *trans)
{
	...
	/* create sec layer */
	self->sec_layer = xrdp_sec_create(self, trans, self->client_info.crypt_level, self->client_info.channel_code);
...
xrdp_rdp_send_data_update_sync(struct xrdp_rdp *self)
{
	...
	make_stream(s);
	init_stream(s, 8192); DEBUG(("in xrdp_rdp_send_data_update_sync"));
...
xrdp_rdp_incoming(struct xrdp_rdp *self)
{
	...
	self->mcs_channel = self->sec_layer->mcs_layer->userid + MCS_USERCHANNEL_BASE;
...
/*****************************************************************************/
static int APP_CC
xrdp_process_capset_general(struct xrdp_rdp *self, struct stream *s, int len)
{
	int i;
...
/*****************************************************************************/
static int APP_CC
xrdp_process_capset_order(struct xrdp_rdp *self, struct stream *s, int len)
{
	int i;
	char order_caps[32];
	int ex_flags;
	...
	DEBUG(("dest blt-0 %d", order_caps[0])); DEBUG(("pat blt-1 %d", order_caps[1])); DEBUG(("screen blt-2 %d", order_caps[2])); DEBUG(("memblt-3-13 %d %d", order_caps[3], order_caps[13])); DEBUG(("triblt-4-14 %d %d", order_caps[4], order_caps[14])); DEBUG(("line-8 %d", order_caps[8])); DEBUG(("line-9 %d", order_caps[9])); DEBUG(("rect-10 %d", order_caps[10])); DEBUG(("desksave-11 %d", order_caps[11])); DEBUG(("polygon-20 %d", order_caps[20])); DEBUG(("polygon2-21 %d", order_caps[21])); DEBUG(("polyline-22 %d", order_caps[22])); DEBUG(("ellipse-25 %d", order_caps[25])); DEBUG(("ellipse2-26 %d", order_caps[26])); DEBUG(("text2-27 %d", order_caps[27])); DEBUG(("order_caps dump"));
...
static int APP_CC
xrdp_process_capset_bmpcache(struct xrdp_rdp *self, struct stream *s, int len)
{
	...
	in_uint16_le(s, self->client_info.cache3_size); DEBUG(("cache1 entries %d size %d", self->client_info.cache1_entries,
					self->client_info.cache1_size)); DEBUG(("cache2 entries %d size %d", self->client_info.cache2_entries,
					self->client_info.cache2_size)); DEBUG(("cache3 entries %d size %d", self->client_info.cache3_entries,
...
/* get the bitmap cache size */
static int APP_CC
xrdp_process_capset_bmpcache2(struct xrdp_rdp *self, struct stream *s, int len)
{
	int Bpp = 0;
	int i = 0;
...
/*****************************************************************************/
static int xrdp_process_capset_cache_v3_codec_id(struct xrdp_rdp *self, struct stream *s, int len)
{
	int codec_id;

	in_uint8(s, codec_id);
	g_writeln("xrdp_process_capset_cache_v3_codec_id: cache_v3_codec_id %d", codec_id);
...
static int APP_CC
xrdp_process_capset_pointercache(struct xrdp_rdp *self, struct stream *s, int len)
{
	int i;
...
static int APP_CC
xrdp_process_capset_brushcache(struct xrdp_rdp *self, struct stream *s, int len)
{
	int code;
...
/*****************************************************************************/
static int APP_CC
xrdp_process_offscreen_bmpcache(struct xrdp_rdp *self, struct stream *s, int len)
{
	int i32;
	...
	g_writeln("xrdp_process_offscreen_bmpcache: support level %d "
		"cache size %d MB cache entries %d", self->client_info.offscreen_support_level,
			self->client_info.offscreen_cache_size, self->client_info.offscreen_cache_entries);
	return 0;
}
...
xrdp_process_capset_rail(struct xrdp_rdp *self, struct stream *s, int len)
{
	...
	g_writeln("xrdp_process_capset_rail: rail_support_level %d", self->client_info.rail_support_level);
	return 0;
}
...
xrdp_process_capset_window(struct xrdp_rdp *self, struct stream *s, int len)
{
	...
	g_writeln("xrdp_process_capset_window wnd_support_level %d "
		"wnd_num_icon_caches %d wnd_num_icon_cache_entries %d", self->client_info.wnd_support_level,
			self->client_info.wnd_num_icon_caches, self->client_info.wnd_num_icon_cache_entries);
	return 0;
}
...
xrdp_process_capset_codecs(struct xrdp_rdp *self, struct stream *s, int len)
{
	...
	for (index = 0; index < codec_count; index++)
	{
		...
		{
			g_writeln("xrdp_process_capset_codecs: nscodec codec id %d prop len %d", codec_id,
					codec_properties_length);
			...
			if (g_memcmp(codec_guid, XR_CODEC_GUID_REMOTEFX, 16) == 0)
			{
				g_writeln("xrdp_process_capset_codecs: rfx codec id %d prop len %d", codec_id,
						codec_properties_length);
				...
				if (g_memcmp(codec_guid, XR_CODEC_GUID_JPEG, 16) == 0)
				{
					g_writeln("xrdp_process_capset_codecs: jpeg codec id %d prop len %d", codec_id,
							codec_properties_length);
...
xrdp_rdp_process_data_input(struct xrdp_rdp *self, struct stream *s)
{
	...
	for (index = 0; index < num_events; index++)
	{
		...
		in_sint16_le(s, param2); DEBUG(("xrdp_rdp_process_data_input event %4.4x flags %4.4x param1 %d \
param2 %d time %d", msg_type, device_flags, param1, param2, time));
		...
		if (self->session->callback != 0)
		{
			...
			self->session->callback(self->session->id, msg_type, param1, param2, device_flags, time);
		}
	}
...
xrdp_rdp_process_data_control(struct xrdp_rdp *self, struct stream *s)
{
	...
	{
		DEBUG(("xrdp_rdp_process_data_control got RDP_CTL_REQUEST_CONTROL")); DEBUG(("xrdp_rdp_process_data_control calling xrdp_rdp_send_synchronise"));
...
xrdp_rdp_process_data(struct xrdp_rdp *self, struct stream *s)
{
	...
	in_uint8(s, ctype);
	in_uint16_le(s, clen); DEBUG(("xrdp_rdp_process_data code %d", data_type));
...
xrdp_rdp_send_deactive(struct xrdp_rdp *self)
{
	...
	if (xrdp_rdp_init(self, s) != 0)
	{
		free_stream(s); DEBUG(("out xrdp_rdp_send_deactive error"));
		return 1;
	}
	...
	free_stream(s); DEBUG(("out xrdp_rdp_send_deactive"));
	return 0;
}
