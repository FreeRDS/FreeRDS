...
xrdp_fastpath_send_update_pdu(struct xrdp_fastpath *self, tui8 updateCode, struct stream *s)
{
	...
	for (i = 0; payloadLeft > 0; i++)
	{
		...
		i32 = (updateCode & 0x0f) | ((fragment & 0x03) << 4) | ((compression & 0x03) << 6);
		out_uint8(s_send, i32);
...
/*****************************************************************************/
int xrdp_fastpath_process_update(struct xrdp_fastpath *self, tui8 updateCode, tui32 size, struct stream *s)
{
	switch (updateCode)
	{
		...
		case FASTPATH_UPDATETYPE_POINTER:
			break;
		default:
			g_writeln("xrdp_fastpath_process_update: unknown updateCode 0x%X", updateCode);
			break;
	}
...
xrdp_fastpath_process_data(struct xrdp_fastpath *self, struct stream *s, tui8 header)
{
	...
	{
		/* TODO */
		g_writeln("xrdp_fastpath_process_data: updateFrag=%d, updateComp=%d", updateFrag, updateComp);
		return 1;
	}
