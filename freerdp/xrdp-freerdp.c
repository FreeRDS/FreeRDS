...
lxrdp_set_param(struct mod *mod, char *name, char *value)
{
	...
	if (g_strcmp(name, "hostname") == 0)
	{
		...
		if (g_strcmp(name, "ip") == 0)
		{
			settings->ServerHostname = g_strdup(value);
		} else
			if (g_strcmp(name, "port") == 0)
			{
				settings->ServerPort = g_atoi(value);
			} else
				if (g_strcmp(name, "keylayout") == 0)
				{
				} else
					if (g_strcmp(name, "name") == 0)
					{
					} else
						if (g_strcmp(name, "lib") == 0)
						{
						} else
							if (g_strcmp(name, "username") == 0)
							{
								...
								if (g_strcmp(name, "password") == 0)
								{
									...
									if (g_strcmp(name, "client_info") == 0)
									{
										g_memcpy(
												&(mod->client_info),
												value,
												sizeof(mod->client_info));
...
/******************************************************************************/
static int 
lxrdp_get_wait_objs(struct mod *mod, tbus *read_objs, int *rcount, tbus *write_objs, int *wcount, int *timeout)
{
	void **rfds;
	void **wfds;
	BOOL ok;
...
/******************************************************************************/
static void 
lfreerdp_cache_bitmapV2(rdpContext *context, CACHE_BITMAP_V2_ORDER *cache_bitmap_v2_order)
{
	char *dst_data;
	char *dst_data1;
	int bytes;
...
/******************************************************************************/
static void 
lfreerdp_pointer_system(rdpContext *context, POINTER_SYSTEM_UPDATE *pointer_system)
{
...
/******************************************************************************/
static void 
lfreerdp_pointer_color(rdpContext *context, POINTER_COLOR_UPDATE *pointer_color)
{
...
/******************************************************************************/
static int 
lfreerdp_convert_color_image(void *dst, int dst_width, int dst_height, int dst_bpp, int dst_delta, void *src,
		int src_width, int src_height, int src_bpp, int src_delta)
{
	...
	for (j = 0; j < dst_height; j++)
	{
		...
		{
			pixel = lfreerdp_get_pixel(src, src_width, src_height, src_bpp, src_delta, i, j);
			lfreerdp_set_pixel(pixel, dst, dst_width, dst_height, dst_bpp, dst_delta, i, j);
		}
	}
...
/******************************************************************************/
static void 
lfreerdp_pointer_new(rdpContext *context, POINTER_NEW_UPDATE *pointer_new)
{
	struct mod *mod;
	int index;
	tui8 *dst;
...
/*****************************************************************************/
void 
lrail_WindowCreate(rdpContext *context, WINDOW_ORDER_INFO *orderInfo, WINDOW_STATE_ORDER *window_state)
{
	int index;
	struct mod *mod;
...
/*****************************************************************************/
void 
lrail_WindowUpdate(rdpContext *context, WINDOW_ORDER_INFO *orderInfo, WINDOW_STATE_ORDER *window_state)
{
...
/*****************************************************************************/
void 
lrail_WindowCachedIcon(rdpContext *context, WINDOW_ORDER_INFO *orderInfo, WINDOW_CACHED_ICON_ORDER *window_cached_icon)
{
	struct mod *mod;
...
/*****************************************************************************/
void 
lrail_NotifyIconCreate(rdpContext *context, WINDOW_ORDER_INFO *orderInfo, NOTIFY_ICON_STATE_ORDER *notify_icon_state)
{
	struct mod *mod;
...
/*****************************************************************************/
void 
lrail_NotifyIconUpdate(rdpContext *context, WINDOW_ORDER_INFO *orderInfo, NOTIFY_ICON_STATE_ORDER *notify_icon_state)
{
...
/*****************************************************************************/
void 
lrail_MonitoredDesktop(rdpContext *context, WINDOW_ORDER_INFO *orderInfo, MONITORED_DESKTOP_ORDER *monitored_desktop)
{
	int index;
	struct mod *mod;
...
/******************************************************************************/
static BOOL 
lfreerdp_authenticate(freerdp *instance, char **username, char **password, char **domain)
{
...
/******************************************************************************/
static BOOL 
lfreerdp_verify_certificate(freerdp *instance, char *subject, char *issuer, char *fingerprint)
{
