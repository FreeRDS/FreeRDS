...
/* returns cache id */
int APP_CC
xrdp_cache_add_bitmap(struct xrdp_cache *self, struct xrdp_bitmap *bitmap, int hints)
{
	int i = 0;
	int j = 0;
	int oldest = 0;
...
xrdp_cache_add_palette(struct xrdp_cache *self, int *palette)
{
	...
	for (i = 0; i < 6; i++)
	{
		if (g_memcmp(palette, self->palette_items[i].palette, 256 * sizeof(int)) == 0)
		{
...
xrdp_cache_add_pointer(struct xrdp_cache *self, struct xrdp_pointer_item *pointer_item)
{
	...
	{
		if (self->pointer_items[i].x == pointer_item->x && self->pointer_items[i].y == pointer_item->y
				&& g_memcmp(self->pointer_items[i].data, pointer_item->data, 32 * 32 * 4) == 0
				&& g_memcmp(self->pointer_items[i].mask, pointer_item->mask, 32 * 32 / 8) == 0
...
/* returns error */
int APP_CC
xrdp_cache_add_os_bitmap(struct xrdp_cache *self, struct xrdp_bitmap *bitmap, int rdpindex)
{
