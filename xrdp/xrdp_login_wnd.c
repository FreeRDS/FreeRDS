...
xrdp_wm_show_edits(struct xrdp_wm *self, struct xrdp_bitmap *combo)
{
	...
	if (mod != 0)
	{
		...
		for (index = 0; index < mod->names->count; index++)
		{
			...
			{
				/* label */
				b = xrdp_bitmap_create(95, DEFAULT_EDIT_H, self->screen->bpp, WND_TYPE_LABEL, self);
				...
				b->parent = self->login_window;
				b->owner = self->login_window;
				b->left = self->login_window->width >= DEFAULT_WND_LOGIN_W ? DEFAULT_WND_LOGIN_W
						- DEFAULT_EDIT_W - 30 : 70;
				b->top = DEFAULT_ELEMENT_TOP + DEFAULT_COMBO_H + 5 + (DEFAULT_EDIT_H + 5) * count;
				b->id = 100 + 2 * count + 1;
				b->pointer = 1;
				b->tab_stop = 1;
...
xrdp_login_wnd_create(struct xrdp_wm *self)
{
	...
	but->parent = self->login_window;
	but->owner = self->login_window;
	but->left = regular ? DEFAULT_WND_LOGIN_W - ((DEFAULT_BUTTON_W + 10) * 2) - 10 : ((log_width - 30)
			- DEFAULT_BUTTON_W);
	but->top = DEFAULT_WND_LOGIN_H - DEFAULT_BUTTON_H - 15;
	but->id = 2;
